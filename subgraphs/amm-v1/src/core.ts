...
export function handleTrade(event: TradeEvent): void {
  ...
  token1DayData.dailyVolumeUSD = token1DayData.dailyVolumeUSD.plus(
    amount1.times(token1DayData.priceUSD)
  );
  token1DayData.save();
...
export function handleTransfer(event: TransferEvent): void {
  ...
  if (event.params.to.toHexString() == BLACKHOLE_ADDRESS) {
    ...
    event.params.from.toHexString() == BLACKHOLE_ADDRESS &&
    !isZapContract(event.params.to.toHexString())
  ) {
  ...
  if (entity.type == "withdraw") {
    ...
    poolParticipant.liquidityProvided = poolParticipant.liquidityProvided.minus(
      LPTokens
    );
    ...
    poolParticipant.liquidityProvided = poolParticipant.liquidityProvided.plus(
      LPTokens
    );
    pair.totalLPToken = pair.totalLPToken.plus(LPTokens);
  } else if (entity.type == "LP-transfer") {
    let poolParticipant2 = PoolParticipant.load(
      event.address.toHexString() + "-" + entity.to.toHexString()
    );
    if (poolParticipant2 === null) {
      poolParticipant2 = new PoolParticipant(
        event.address.toHexString() + "-" + entity.to.toHexString()
      ) as PoolParticipant;
    ...
    poolParticipant2.liquidityProvided = poolParticipant2.liquidityProvided.plus(
      LPTokens
    );
    poolParticipant.liquidityProvided = poolParticipant.liquidityProvided.minus(
      LPTokens
  ...
  if (!reserveResult.reverted) {
    reserveUSD = convertTokenToDecimal(
  ...
  if (entity.type == "withdraw") {
    ...
    pairDayData.reserve1WithdrawUSD = pairDayData.reserve1WithdrawUSD.plus(
      reserve1DiffUSD
    );
  } else if (
    entity.type == "two-sided-deposit" ||
    ...
    pairDayData.reserve1DepositUSD = pairDayData.reserve1DepositUSD.plus(
      reserve1DiffUSD
    );
  }
  pairDayData.save();
