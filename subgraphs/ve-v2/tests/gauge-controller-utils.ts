...
import {
  gaugeControllerCommitOwnership,
  gaugeControllerApplyOwnership,
  AddType,
  NewTypeWeight,
  NewGaugeWeight,
  VoteForGauge,
...
export function creategaugeControllerCommitOwnershipEvent(
  admin: Address
): gaugeControllerCommitOwnership {
  let gaugeControllerCommitOwnershipEvent = changetype<
    gaugeControllerCommitOwnership
  >(newMockEvent())

  gaugeControllerCommitOwnershipEvent.parameters = new Array()

  gaugeControllerCommitOwnershipEvent.parameters.push(
    new ethereum.EventParam("admin", ethereum.Value.fromAddress(admin))
  )

  return gaugeControllerCommitOwnershipEvent
}

export function creategaugeControllerApplyOwnershipEvent(
  admin: Address
): gaugeControllerApplyOwnership {
  let gaugeControllerApplyOwnershipEvent = changetype<
    gaugeControllerApplyOwnership
  >(newMockEvent())

  gaugeControllerApplyOwnershipEvent.parameters = new Array()

  gaugeControllerApplyOwnershipEvent.parameters.push(
    new ethereum.EventParam("admin", ethereum.Value.fromAddress(admin))
  )

  return gaugeControllerApplyOwnershipEvent
}
