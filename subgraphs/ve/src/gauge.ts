...
import { Gauge, GaugeReward } from "../generated/schema";
...
function _updateDfxBalance(gauge: Gauge): void {
...
function _updateRewardsAvailable(gauge: Gauge): void {
  ...
  for (let i: i32 = 0; i < numRewards; i++) {
    const gaugeContract = LiquidityGaugeV4.bind(Address.fromString(gauge.id));
...
export function _updateWeights(gauge: Gauge): void {
