...
import {
  ChildChainRegistry as ChildChainRegistryContract,
  RegisterGaugeSet as RegisterGaugeSetEvent,
  RecalculateSubgraph as RecalculateSubgraphEvent,
} from "../generated/GaugeRegistry/ChildChainRegistry";
import {
...
import {
  ...
  getReceiver,
  getStreamer,
  mirrorGaugeAttributes,
} from "./gauge-helpers";

export function handleNewGaugeSet(event: RegisterGaugeSetEvent): void {
  ...
  gaugeSet.registry = event.address.toHexString();
...
// Use event to manually force a re-calculation of available rewards
export function handlePoke(event: RecalculateSubgraphEvent): void {
  const registry = ChildChainRegistryContract.bind(event.address);
  const activeRootGauges = registry.activeRootGauges();
  for (let i = 0; i < activeRootGauges.length; i++) {
