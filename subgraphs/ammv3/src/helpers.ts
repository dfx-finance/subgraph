...
import {Config} from "../generated/Config/Config";
import { 
    DFX_TC_ADDRESS,
    ZAP_ADDRESS_V25,
    BLACKHOLE_ADDRESS,
...
export function fetchAssimilator(assimFactoryAddress: Address, tokenAddress: Address, quoteAddress : Address): Address {
    ...
    if(!assimResult.reverted)
        assim = assimResult.value
    return assim
}
...
export function fetchToken(curveAddress: Address, derivative: BigInt): Address {
    ...
    if(!derivtResult.reverted)
        derivt = derivtResult.value
    return derivt
}
...
export function fetchTokenSymbol(tokenAddress: Address): string {
    ...
    if(!tokenSymbolResult.reverted)
        tokenSymbol = tokenSymbolResult.value
    return tokenSymbol
}
...
export function fetchTokenName(tokenAddress: Address): string {
    ...
    if(!tokenNameResult.reverted)
        tokenName = tokenNameResult.value
    return tokenName
}
...
export function fetchProtocolAlpha(curveAddress: Address): BigInt {
    ...
    if(!alphaResult.reverted)
        alpha = alphaResult.value.value0
    return alpha
}
...
export function fetchProtocolBeta(curveAddress: Address): BigInt {
    ...
    if(!betaResult.reverted)
        beta = betaResult.value.value1
    return beta
}
...
export function fetchProtocolDelta(curveAddress: Address): BigInt {
    ...
    if(!deltaResult.reverted)
        delta = deltaResult.value.value2
    return delta
}
...
export function fetchProtocolEpsilon(curveAddress: Address): BigInt {
    ...
    if(!epsionResult.reverted)
        epsilon = epsionResult.value.value3
    return epsilon
}
...
export function fetchProtocolLambda(curveAddress: Address): BigInt {
    ...
    if(!lambdaResult.reverted)
        lambda = lambdaResult.value.value4
    return lambda
}
...
export function fetchPriceFromAssimilator(assimilatorAddress: Address): BigDecimal {
    ...
    if(!rawPriceResult.reverted)
        rawPrice = rawPriceResult.value
    let decimalResult = contract.try_oracleDecimals()
    if(!decimalResult.reverted)
        decimal = decimalResult.value
...
export function fetchTokenPriceInUSD(oracleAddr : Address): BigDecimal {
    ...
    if(!rawPriceResult.reverted && !decimalResult.reverted)
        return convertTokenToDecimal(rawPriceResult.value, BigInt.fromI32(decimalResult.value))
...
export function handleNewPoolParticipant(
    ...
    if (poolParticipant === null) {
        ...
        if (transferType == "withdraw") {
            ...
            // Check balance first
            let balance = fetchBalanceOf(Token.bind( poolAddress), participantAddresss)
            if (balance <= ZERO_BD) {
